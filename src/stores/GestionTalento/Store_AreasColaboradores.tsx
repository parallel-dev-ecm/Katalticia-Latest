...
const useAreasStore = create<State>((set, get) => ({
  allPuestos: [],

  readAllPuestos: async () => {
    try {
      const response = await axiosInstance.get("/areas/getAll");
  ...
  addPuesto: async (cC: CategoriaColaborador) => {
    try {
      const response = await axiosInstance.post("/areas/postArea", cC);
      return true;
    } catch (error) {
